...
1. **Arnoldi Iteration** implementation and testing: in the `Arnoldi_Iteration/` folder :open_file_folder: there is a ipynb notebook where has been implemented the Arnoldi Iteration algorithm. <br>
...
2. **GMRES** algorithm implementation and testing: in the `GMRES/` folder :open_file_folder: there is a ipynb notebook where has been implemented the GMRES algorithm. This time the code is written in `Octave` so, in order to execute the code in you machine, you can follow the [Installation and Testing Guide](#installation-and-virtual-environment-preparation) in the section below. The notebook is also formattend in an Ocatve file so, you can execute only that without install any docker container. <br>
